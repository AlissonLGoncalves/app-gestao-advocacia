...
def test_get_eventos_lista_vazia(auth_client, db):
    response = auth_client.get("/api/v1/eventos")
...
def test_create_evento_sucesso(auth_client, db):
    ...
    response = auth_client.post("/api/v1/eventos", json=payload)
...
def test_create_evento_dados_incompletos(auth_client, db):
    response = auth_client.post("/api/v1/eventos", json={"titulo": "Sem data"})
    assert response.status_code == 400


def test_get_evento_especifico_existente(auth_client, db):
    res_post = auth_client.post(
        "/api/v1/eventos", json={"titulo": "Evento GET", "data_inicio": _inicio_futuro()}
    )
    assert res_post.status_code == 201
    evento_id = json.loads(res_post.data)["id"]

    response = auth_client.get(f"/api/v1/eventos/{evento_id}")
    assert response.status_code == 200
    data = json.loads(response.data)
...
def test_update_evento_sucesso(auth_client, db):
    res_post = auth_client.post(
        "/api/v1/eventos", json={"titulo": "Evento Update", "data_inicio": _inicio_futuro()}
    )
    assert res_post.status_code == 201
    ...
    response = auth_client.put(f"/api/v1/eventos/{evento['id']}", json=payload)
    assert response.status_code == 200
    data = json.loads(response.data)
    assert data["title"] == payload["titulo"]
...
def test_delete_evento_sucesso(auth_client, db):
    res_post = auth_client.post(
        "/api/v1/eventos", json={"titulo": "Evento Delete", "data_inicio": _inicio_futuro()}
    )
    assert res_post.status_code == 201
    evento_id = json.loads(res_post.data)["id"]

    response_delete = auth_client.delete(f"/api/v1/eventos/{evento_id}")
