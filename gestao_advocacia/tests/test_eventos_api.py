...
def test_get_evento_especifico_existente(auth_client, db):
    res_post = auth_client.post('/api/eventos', json={"titulo": "Evento GET", "data_inicio": _inicio_futuro()})
...
def test_update_evento_sucesso(auth_client, db):
    res_post = auth_client.post('/api/eventos', json={"titulo": "Evento Update", "data_inicio": _inicio_futuro()})
    assert res_post.status_code == 201
...
def test_delete_evento_sucesso(auth_client, db):
    res_post = auth_client.post('/api/eventos', json={"titulo": "Evento Delete", "data_inicio": _inicio_futuro()})
    assert res_post.status_code == 201
    evento_id = json.loads(res_post.data)['id']
