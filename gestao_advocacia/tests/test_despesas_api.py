...
def test_get_despesas_lista_vazia(auth_client, db):
    response = auth_client.get('/api/despesas')
...
def test_create_despesa_com_caso_sucesso(auth_client, db):
    ...
    response = auth_client.post('/api/despesas', json=payload)
...
def test_update_despesa_sucesso(auth_client, db):
    ...
    res_post = auth_client.post('/api/despesas', json={**DESPESA_BASE, "caso_id": caso_id})
    ...
    response = auth_client.put(f'/api/despesas/{despesa_id}', json=payload)
    assert response.status_code == 200
    data = json.loads(response.data)
...
def test_delete_despesa_sucesso(auth_client, db):
    ...
    response_delete = auth_client.delete(f'/api/despesas/{despesa_id}')
    assert response_delete.status_code == 204

    response_get = auth_client.get(f'/api/despesas/{despesa_id}')
