...
def register_eventos_routes(app, eventos_ns, evento_input_model_dto, evento_model_dto):
    ...
    class EventoListAPI(Resource):
        ...
        def post(self):
            ...
            try:
                ...
                return {'message': 'Formato de data invalido. Utilize o formato ISO 8601 (ex: YYYY-MM-DDTHH:MM:SS).'}, 400
            ...
            db.session.commit()
            app.logger.info(f"Novo evento '{novo_evento.titulo}' (ID: {novo_evento.id}) criado para usuario ID {user_id}.")
    ...
    class EventoDetailAPI(Resource):
        ...
        def put(self, evento_id_param):
            ...
            data = request.get_json()
            if not data.get('titulo') or not data.get('data_inicio'):
                return {'message': 'Titulo e data de inicio sao obrigatorios para atualizacao do evento.'}, 400
            try:
                data_inicio_obj = datetime.fromisoformat(data['data_inicio'])
                data_fim_obj = datetime.fromisoformat(data['data_fim']) if data.get('data_fim') else None
            except ValueError:
        ...
        def delete(self, evento_id_param):
            ...
            db.session.commit()
            app.logger.info(f"Evento ID {evento.id} ('{evento.titulo}') deletado pelo usuario ID {user_id}.")
            return '', 204
