...
def register_casos_routes(app, casos_ns, caso_input_model_dto, caso_model_dto, movimentacao_cnj_output_model_dto):
            ...
            if movimentos and isinstance(movimentos, list):
                try:
                    ...
                    movimento_mais_recente = {
                        ...
                        'descricao': mov0.get('nome') or mov0.get('movimentoNacional', {}).get('descricao'),
                    }
                except Exception:
                    movimento_mais_recente = None
    ...
    class CasoListAPI(Resource):
        ...
        def post(self):
            ...
            if num_proc_strip and query_for_tenant(Caso).filter_by(numero_processo=num_proc_strip).first():
                return {'message': f"Já existe um caso com número de processo '{num_proc_strip}'."}, 409
    ...
    class CasoAtualizarCNJAPI(Resource):
        @casos_ns.doc('atualizar_caso_via_cnj_endpoint', security='jsonWebToken', description='Consulta a API do CNJ para um caso específico, buscando as últimas movimentações e atualizando o status do caso e registrando novas movimentações no sistema local.')
        @jwt_required()
        @tenant_scoped
            ...
            try:
                ...
                for movimento_json in movimentos_api_cnj:
                    ...
                    if not data_mov_str_api:
                        continue
                    try:
                        data_mov_obj_utc = datetime.fromisoformat(data_mov_str_api.replace('Z', '+00:00'))
                    except ValueError:
                    ...
                    mov_existente = MovimentacaoCNJ.query.filter_by(caso_id=caso_para_atualizar.id, data_movimentacao=data_mov_obj_utc).filter(MovimentacaoCNJ.descricao.startswith(descricao_db[:150])).first()
    ...
    class CasoListarMovimentacoesCNJAPI(Resource):
        ...
        def get(self, caso_id):
            ...
            movimentacoes = MovimentacaoCNJ.query.filter_by(caso_id=caso_db.id).order_by(MovimentacaoCNJ.data_movimentacao.desc(), MovimentacaoCNJ.id.desc()).all()
