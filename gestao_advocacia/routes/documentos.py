...
def register_documentos_routes(app, documentos_ns, documento_model_dto):
    ...
    class DocumentoListAPI(Resource):
        ...
        @documentos_ns.doc(security='jsonWebToken', description="Lista documentos do usuario, com filtro opcional por 'caso_id'.")
    ...
    class DocumentoUploadAPI(Resource):
        @jwt_required()
        @tenant_scoped
        @documentos_ns.doc(security='jsonWebToken', description="Faz upload de um novo documento. Use 'multipart/form-data'. Campo 'file' para o arquivo e opcionalmente 'caso_id' no formulario.")
        ...
        def post(self):
            ...
            if 'file' not in request.files:
                return {'message': 'Nenhum arquivo foi incluido na requisicao (campo "file" ausente).'}, 400
            ...
            if file_storage and is_allowed_file_upload(file_storage.filename):
                ...
                if caso_id_from_form:
                    try:
                        ...
                        if not query_for_tenant(Caso).filter_by(id=db_caso_id).first():
                            ...
                            return {'message': f'Caso com ID {db_caso_id} nao encontrado ou nao pertence ao usuario.'}, 400
                    except ValueError:
                        os.remove(full_file_path_to_save)
                ...
                db.session.commit()
                app.logger.info(f"Documento '{novo_documento_db.nome_arquivo}' (ID: {novo_documento_db.id}) salvo para usuario ID {user_id}.")
                doc_dict = novo_documento_db.to_dict()
                return doc_dict, 201
            return {'message': 'Tipo de arquivo nao permitido. Extensoes permitidas: ' + ', '.join(ALLOWED_EXTENSIONS_UPLOAD)}, 400
    ...
    class DocumentoDownloadAPI(Resource):
        @jwt_required()
        @tenant_scoped
        @documentos_ns.doc(security='jsonWebToken', description='Permite o download de um documento especifico.')
        ...
        def get(self, doc_id_param):
            ...
            if not os.path.exists(documento_db.path_arquivo):
                app.logger.error(f"Arquivo para Doc ID {doc_id_param} nao encontrado em '{documento_db.path_arquivo}'.")
        ...
        def delete(self, doc_id_param):
            ...
            try:
                if os.path.exists(file_path_on_disk):
                    ...
                    app.logger.warning(f"Arquivo fisico '{file_path_on_disk}' para Doc ID {doc_id_param} nao encontrado durante exclusao.")
            except Exception as e_delete_file:
                app.logger.error(f"Erro ao deletar arquivo fisico '{file_path_on_disk}' para Doc ID {doc_id_param}: {str(e_delete_file)}")
            db.session.delete(documento_db)
            db.session.commit()
            app.logger.info(f"Documento ID {doc_id_param} ('{document_name_log}') deletado pelo usuario ID {user_id}.")
            return '', 204
