...
def job_verificar_processos_cnj():
    ...
    try:
        ...
        logger.critical(
            f"JOB CNJ: Falha crítica ao importar db/modelos de 'app' dentro do job: {e}. O job não pode continuar. Verifique a estrutura do projeto e os caminhos de importação."
        )
        return
    ...
    if not current_app.config.get("CNJ_JOB_ENABLED", False):
        logger.info(
            f"JOB CNJ [{datetime.now(tz=current_app.config.get('SCHEDULER_TIMEZONE'))}]: Job está DESABILITADO nas configurações. Pulando execução."
        )
        return

    logger.info(
        f"JOB CNJ [{datetime.now(tz=current_app.config.get('SCHEDULER_TIMEZONE'))}]: Iniciando verificação de processos no CNJ..."
    )
    ...
    try:
        ...
        with current_app.app_context():
            casos_para_verificar = (
                Caso.query.filter(
                    Caso.numero_processo.isnot(None),
                    Caso.numero_processo != "",
                    db.or_(
                        Caso.data_ultima_verificacao_cnj.is_(None),
                        Caso.data_ultima_verificacao_cnj < limite_tempo_verificacao,
                    ),
                )
                .order_by(Caso.data_ultima_verificacao_cnj.asc().nulls_first())
                .limit(max_casos_por_execucao)
                .all()
            )

        logger.info(
            f"JOB CNJ: Encontrados {len(casos_para_verificar)} casos para verificação (limite: {max_casos_por_execucao})."
        )
        ...
        for caso_item in casos_para_verificar:
            logger.info(
                f"JOB CNJ: Verificando caso ID {caso_item.id}, processo '{caso_item.numero_processo}'"
            )
            try:
                ...
                with current_app.app_context():
                    ...
                    if status_code < 400:  # Sucesso na consulta
                        ...
                        if hits:
                            ...
                            if not isinstance(movimentos_api, list):
                                movimentos_api = []
                            ...
                            movimentos_api.sort(
                                key=lambda m: m.get("dataHora", "1900-01-01T00:00:00Z"),
                                reverse=True,
                            )

                            for mov_json_job in movimentos_api:
                                data_mov_str_job = mov_json_job.get("dataHora")
                                if not data_mov_str_job:
                                    continue

                                try:
                                    data_mov_obj_job = datetime.fromisoformat(
                                        data_mov_str_job.replace("Z", "+00:00")
                                    )
                                except ValueError:
                                    logger.warning(
                                        f"JOB CNJ: Formato de 'dataHora' ('{data_mov_str_job}') inválido para caso {caso_item.id}. Movimento ignorado."
                                    )
                                    continue
                                ...
                                if not descricao_db_job:
                                    descricao_db_job = (
                                        mov_json_job.get("descricao")
                                        or mov_json_job.get("nome")
                                        or str(
                                            mov_json_job.get("codigoNacional", {}).get(
                                                "codigo", "Movimento"
                                            )
                                        )
                                    )
                                ...
                                mov_existente_job = (
                                    MovimentacaoCNJ.query.filter_by(
                                        caso_id=caso_item.id, data_movimentacao=data_mov_obj_job
                                    )
                                    .filter(
                                        MovimentacaoCNJ.descricao.startswith(descricao_db_job[:150])
                                    )
                                    .first()
                                )
                                ...
                                if not mov_existente_job:
                                    ...
                                    if (
                                        data_mov_recente_job is None
                                        or data_mov_obj_job > data_mov_recente_job
                                    ):
                            ...
                            logger.info(
                                f"JOB CNJ: Caso {caso_item.id} processado, {novas_movs_job_count} nova(s) movimentação(ões) registrada(s)."
                            )
                        else:
                            logger.info(
                                f"JOB CNJ: Nenhuma informação (hit) encontrada no CNJ para caso {caso_item.id} (processo {caso_item.numero_processo})."
                            )

                    elif status_code >= 400:
                        logger.error(
                            f"JOB CNJ: Erro ao consultar CNJ para caso {caso_item.id}. Status: {status_code}, Erro: {dados_cnj_raw.get('erro')}"
                        )
                ...
                logger.error(
                    f"JOB CNJ: Exceção ao processar caso ID {caso_item.id} (processo '{caso_item.numero_processo}'): {str(e_job_item_proc)}",
                    exc_info=True,
                )
                db.session.rollback()
                try:
                    ...
                    with current_app.app_context():
                        ...
                        if caso_item_reattached:
                            ...
                            logger.error(
                                f"JOB CNJ: Não foi possível re-attachar caso ID {caso_item.id} para atualizar data_ultima_verificacao_cnj após erro."
                            )
                except Exception as e_commit_on_error:
                    logger.error(
                        f"JOB CNJ: Falha crítica ao tentar atualizar data_ultima_verificacao_cnj APÓS ERRO para caso {caso_item.id}: {str(e_commit_on_error)}"
                    )
                    db.session.rollback()
        ...
        logger.info(
            f"JOB CNJ [{datetime.now(tz=current_app.config.get('SCHEDULER_TIMEZONE'))}]: Verificação de processos concluída."
        )

    except Exception as e_job_geral:
        logger.critical(
            f"JOB CNJ [{datetime.now(tz=current_app.config.get('SCHEDULER_TIMEZONE'))}]: Erro CRÍTICO durante a execução do job: {str(e_job_geral)}",
            exc_info=True,
        )
