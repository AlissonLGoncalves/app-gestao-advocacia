...
function rodape(doc) {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    doc.text(
      `Página ${i} de ${total} — Gerado por Patronus App`,
      PW / 2, PH - 8,
      { align: 'center' }
    );
  }
 ...
 */
function bloco(doc, texto, y, { fontSize = 11, bold = false, center = false, indent = 0, extra = 3 } = {}) {
...
function fmtAdvogado(opcoes) {
  ...
  const oab = opcoes.oab ? `OAB/${opcoes.estadoOAB || 'UF'} nº ${opcoes.oab}` : `inscrito(a) na OAB/${opcoes.estadoOAB || 'UF'} sob nº ___________`;
...
function gerarContratoRevisao(doc, cliente, opcoes) {
  ...
  y = bloco(doc,
    `${fmtAdvogado(opcoes)}, denominado(a) simplesmente CONTRATADO(A).`,
    y, { fontSize: 10.5, bold: true }
  );
...
function gerarContratoAposentadoria(doc, cliente, opcoes) {
  ...
  y = secao(doc, 'Contratado(a)', y);
  y = bloco(doc,
    `${fmtAdvogado(opcoes)}, denominado(a) CONTRATADO(A).`,
    y, { fontSize: 10.5, bold: true }
  );
...
function gerarDeclaracaoHipossuficiencia(doc, cliente, opcoes) {
  ...
  y = bloco(doc,
    '3 — Possuo ___ (___________) dependentes;',
    y, { fontSize: 10.5, indent: 5 }
  );
...
function gerarDeclaracaoResidencia(doc, cliente, opcoes) {
  ...
  y = espaco(y, 3);
  y = bloco(doc,
    `Endereço: ${fmtEndereco(cliente)}`,
    y, { fontSize: 11, bold: true, indent: 5 }
  );
