...
function useClienteForm({ formData, isEditing, clienteParaEditar, onClienteChange, setLoading }) {
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.cpf_cnpj || !formData.cpf_cnpj.trim()) {
      ...
      if (formData.tipo_pessoa === 'PF' && numCpfCnpj.length !== 11) errors.cpf_cnpj = 'CPF principal deve conter 11 digitos.';
      if (formData.tipo_pessoa === 'PJ' && numCpfCnpj.length !== 14) errors.cpf_cnpj = 'CNPJ principal deve conter 14 digitos.';
    }
  ...
  }, [clearValidationErrors, clienteParaEditar, formData, isEditing, onClienteChange, setLoading, validateForm]);
