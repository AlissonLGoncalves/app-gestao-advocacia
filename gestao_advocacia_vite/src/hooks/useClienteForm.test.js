...
describe('useClienteForm', () => {
  ...
  it('nao gera erro de email quando email valido', async () => {
    ...
    const { result } = renderHook(() =>
      useClienteForm(makeProps({ formData: makeFormData({ email: 'ok@empresa.com.br' }), setLoading }))
    );
  ...
  it('nao chama setLoading quando CPF PF com digitos incorretos', async () => {
    const setLoading = vi.fn();
    const { result } = renderHook(() =>
      useClienteForm(makeProps({ formData: makeFormData({ tipo_pessoa: 'PF', cpf_cnpj: '123.456.789' }), setLoading }))
    );
