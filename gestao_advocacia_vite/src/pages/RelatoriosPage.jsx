        ...
        <div className="table-responsive">
          <table className="table table-sm table-hover align-middle">
            ...
            <tbody>
              {data.items.map((item) => (
                <tr key={item.id} className={item.status === 'Vencido' ? 'table-danger' : ''}>
                  ...
                  <td>
                    <span className={`badge ${item.status === 'Pendente' ? 'bg-warning text-dark' : item.status === 'Vencido' ? 'bg-danger' : 'bg-success'}`}>
                      {item.status}
                    </span>
                  </td>
                </tr>
            ...
            <tfoot className="table-light fw-bold">
              <tr>
                <td colSpan={3} className="text-end">Total pendente:</td>
                <td className="text-end">{fmtBRL(data.total_geral)}</td>
                <td colSpan={2} className="text-muted small">({data.quantidade_items} item(s))</td>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      {data.items.length === 0 ? (
        ...
        <div className="table-responsive">
          <table className="table table-sm table-hover align-middle">
            ...
            <tbody>
              {data.items.map((item) => (
                <tr key={item.id} className={item.status === 'Vencida' ? 'table-danger' : ''}>
                  ...
                  <td className="small">{fmtData(item.data_vencimento)}</td>
                  <td>
                    <span className={`badge ${item.status === 'A Pagar' ? 'bg-warning text-dark' : item.status === 'Vencida' ? 'bg-danger' : 'bg-success'}`}>
                      {item.status}
                    </span>
                  </td>
                </tr>
            ...
            <tfoot className="table-light fw-bold">
              <tr>
                <td colSpan={2} className="text-end">Total pendente:</td>
                <td className="text-end">{fmtBRL(data.total_geral)}</td>
                <td colSpan={2} className="text-muted small">({data.quantidade_items} item(s))</td>
              </tr>
            </tfoot>
    ...
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        ...
        <div className="d-flex align-items-center gap-2">
          <button className="btn btn-sm btn-outline-secondary" onClick={() => setAno((a) => a - 1)}>◀</button>
          <span className="fw-bold">{ano}</span>
          <button className="btn btn-sm btn-outline-secondary" onClick={() => setAno((a) => a + 1)} disabled={ano >= anoAtual}>▶</button>
        </div>
      </div>
      ...
      <div className="row g-3 mb-4">
        {[
          ...
          { label: 'Resultado do Ano', valor: data.totais.saldo, cor: data.totais.saldo >= 0 ? 'primary' : 'danger' },
        ].map((card) => (
          ...
          <tbody>
            {data.meses.map((m) => (
              <tr key={m.mes} className={m.saldo < 0 ? 'table-danger bg-opacity-25' : ''}>
                ...
                <td className={`text-end text-muted small ${m.saldo_acumulado < 0 ? 'text-danger' : ''}`}>
                  ...
                  <div className="d-flex gap-1 align-items-center" style={{ height: '16px' }}>
                    {m.receitas > 0 && (
                      <div
                        ...
                        style={{ width: `${Math.round((m.receitas / maxVal) * 80)}px`, height: '10px', minWidth: '2px' }}
                    ...
                    {m.despesas > 0 && (
                      <div
                        ...
                        style={{ width: `${Math.round((m.despesas / maxVal) * 80)}px`, height: '10px', minWidth: '2px' }}
                  ...
                  <div className="card-body py-3">
                    <div className="d-flex justify-content-between align-items-start">
                      <div>
                        ...
                        <p className={`fw-bold fs-4 text-${cor === 'light' ? 'dark' : cor} mb-0`}>{g.count}</p>
                      </div>
                      <span className={`badge bg-${cor === 'light' ? 'secondary' : cor} opacity-75 fs-6`}>
                        {g.percentual}%
                      </span>
                    </div>
...
const RELATORIOS = [
  { id: 'contas_receber', label: 'Contas a Receber', icon: 'bi-arrow-down-circle-fill', cor: 'success' },
...
function RelatoriosPage() {
  ...
  const renderRelatorio = () => {
    switch (ativo) {
      case 'contas_receber': return <ContasAReceber />
      case 'contas_pagar': return <ContasAPagar />
      case 'fluxo_caixa': return <FluxoCaixa />
      case 'casos_status': return <CasosStatus />
      default: return (
  ...
  return (
    <div className="container-fluid p-4" style={{ backgroundColor: '#f8fafc', minHeight: 'calc(100vh - 70px)' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <p className="text-muted small mb-4">Análises financeiras e operacionais do seu escritório.</p>
