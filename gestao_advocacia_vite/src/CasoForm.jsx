...
function CasoForm({ casoParaEditar, onCasoChange, onCancel, clienteIdInicial }) {
  ...
  const { validationErrors, setValidationErrors, clearValidationErrors, handleSubmit } = useCasoForm({
    formData,
    isEditing,
    casoParaEditar,
    onCasoChange,
  ...
  useEffect(() => {
    ...
    if (casoParaEditar && casoParaEditar.id) {
      ...
      dadosEdit.data_distribuicao = typeof dadosEdit.data_distribuicao === 'string' ? dadosEdit.data_distribuicao.split('T')[0] : '';
  ...
  const handleMagicUpload = (e) => {
    ...
    xhr.upload.onprogress = (event) => {
      if (event.lengthComputable) setMagicUploadProgress(Math.round((event.loaded / event.total) * 100));
    };
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          ...
          if (jsonRes.dados) {
            ...
            if (jsonRes.dados.numero_processo && jsonRes.dados.numero_processo.length === 25) buscarDadosDataJud(jsonRes.dados.numero_processo);
          } else {
    ...
    <div className="card shadow-sm mb-4">
      ...
      <div className="card-body p-4">
        <div className="alert alert-secondary border-dashed mb-4" style={{ border: '2px dashed #6c757d', backgroundColor: '#f8f9fa' }} role="alert">
          ...
          <p className="small mb-2">Anexe a copia integral do processo para extrair titulo, valor e numero do CNJ automaticamente.</p>

          {isMagicLoading && (
            <div className="mb-3">
              <div className="d-flex justify-content-between small text-muted mb-1">
                <span>{magicUploadProgress === 100 ? 'Processando Automacao IA (Aguarde...)' : 'Enviando...'}</span>
          ...
          <button type="button" className="btn btn-outline-primary shadow-sm rounded-pill btn-sm" onClick={() => magicFileRef.current?.click()} disabled={isMagicLoading}>
