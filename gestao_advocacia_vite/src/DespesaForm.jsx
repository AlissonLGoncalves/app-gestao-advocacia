...
function DespesaForm({ despesaParaEditar, onDespesaChange, onCancel }) {
        ...
        if (casoOriginal && casoOriginal.cliente_id) {
          ...
          if (token) {
            api
              ...
              .catch((err) =>
                console.warn(
                  'DespesaForm: Não foi possível determinar o cliente do caso para edição.',
                  err
                )
              )
          }
  ...
  const categoriasDespesa = [
    ...
    'Impostos e Taxas Escritório',
    'Outras Despesas',
  ]
  const statusDespesa = ['A Pagar', 'Paga', 'Vencida', 'Cancelada']
  const formasPagamentoDespesa = [
    'PIX',
        ...
        <form onSubmit={handleSubmit}>
          <div className="row">
            <div className="col-md-6 mb-3">
              <label htmlFor="selectedClienteIdDesp" className="form-label form-label-sm">
                Filtrar Casos por Cliente (Opcional)
              </label>
              <select
                name="selectedClienteId"
                id="selectedClienteIdDesp"
            ...
            <div className="col-md-6 mb-3">
              ...
              {selectedClienteId &&
                casos.filter((c) => String(c.cliente_id) === selectedClienteId).length === 0 && (
                  <small className="form-text text-muted">
          ...
          <div className="row">
            ...
            <div className="col-md-6 mb-3">
              <label htmlFor="data_despesa_desp" className="form-label form-label-sm">
                Data da Despesa/Pagamento
              </label>
              <input
                type="date"
                name="data_despesa"
                id="data_despesa_desp"
