...
function DespesaForm({ despesaParaEditar, onDespesaChange, onCancel }) {
    ...
    const [clientes, setClientes] = useState([]); // Para popular o select de clientes (para filtrar casos)
    const [casos, setCasos] = useState([]); // Casos filtrados pelo cliente selecionado, ou todos
    const [selectedClienteId, setSelectedClienteId] = useState(''); // Para controlar o filtro de casos
    ...
    // Busca clientes para o select de filtro de casos
    ...
    const fetchCasos = useCallback(async (clienteId = null) => {
        ...
        } // Se clienteId for nulo, busca todos os casos ativos (ou todos, dependendo da API)
        try {
    ...
    useEffect(() => {
        fetchClientes();
        // Carrega casos baseado no cliente selecionado para edição, ou todos se for novo
        if (despesaParaEditar && despesaParaEditar.caso_id) {
            // Precisamos encontrar o cliente_id do caso para popular o filtro de cliente corretamente
            const fetchCasoDoCliente = async () => {
                try {
                    ...
                    console.error("Erro ao buscar cliente do caso para edição", e);
                    fetchCasos();
                }
            };
            fetchCasoDoCliente();
    ...
    useEffect(() => {
        ...
        if (despesaParaEditar) {
            ...
            // caso_id pode ser null, então o || '' é importante para o select
    ...
    const handleChange = (e) => {
        ...
        if (name === "selectedClienteId") { // Nome do estado para o filtro de cliente
    ...
    const handleSubmit = async (e) => {
        ...
        const dadosParaEnviar = {
            ...
            caso_id: formData.caso_id ? parseInt(formData.caso_id, 10) : null, // Envia null se vazio
        ...
        try {
            ...
            if (!isEditing) {
                setFormData(initialState);
                setSelectedClienteId(''); // Reseta filtro de cliente também
            }
        } catch (error) {
    ...
    const categoriasDespesa = [
        "Custas Processuais", "Honorários Periciais", "Cópias e Impressões", "Deslocamento", 
        "Aluguel Escritório", "Contas (Água, Luz, Internet)", "Software e Assinaturas", 
        "Material de Escritório", "Marketing e Publicidade", "Impostos e Taxas", "Outras Despesas"
    ];
                    ...
                    <div className="row">
                        <div className="col-md-6 mb-3">
                            ...
                            <select 
                                ...
                                onChange={handleChange} // Usa o handleChange genérico, que chama setSelectedClienteId
                            >
