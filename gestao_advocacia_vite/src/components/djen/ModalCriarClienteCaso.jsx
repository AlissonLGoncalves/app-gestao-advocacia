...
function calcularPosicaoInicial() {
  ...
  const larguraPainel = Math.min(800, window.innerWidth - 32)
  const x = Math.max(16, Math.floor((window.innerWidth - larguraPainel) / 2))
  return { x, y: 80 }
}
...
export default function ModalCriarClienteCaso({ publicacao, onClose, onSuccess }) {
  ...
  // Painel arrastavel — posicao persistida na sessao
  const { panelRef, headerRef, position, isDragging } = useArrastavel({
    initialPosition: calcularPosicaoInicial(),
  ...
  useEffect(() => {
    ...
    if (primeiroCampo && typeof primeiroCampo.focus === 'function') {
      try {
        ...
        // ignore — foco e best effort
      }
    }
    return () => {
      if (
        elementoOriginal &&
        typeof elementoOriginal.focus === 'function' &&
        document.contains(elementoOriginal)
      ) {
        try {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-labelledby="painel-criar-titulo"
      style={{ left: position.x, top: position.y }}
    >
      <div
        ref={headerRef}
        className={`painel-header${isDragging ? ' dragging' : ''}`}
        tabIndex={0}
        aria-label="Cabeçalho do painel — Alt+Shift+setas para mover"
      >
        <h5 id="painel-criar-titulo" className="modal-title m-0">
