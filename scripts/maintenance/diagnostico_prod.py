...
def main():
    ...
    try:
        with engine.connect() as conn:
            ...
            # Lista de usuários para inspeção
            print('\nUSUÁRIOS (id, username, email, tenant_id):')
            ...
            if res2:
                for r in res2:
                    print(f" id={r['id']} | {r['nome_razao_social']} | cpf={r['cpf_cnpj']} | user_id={r['user_id']}")
            else:
            ...
            if args.cpf:
                ...
                q3 = text('SELECT id, nome_razao_social, cpf_cnpj, tenant_id, user_id FROM cliente WHERE cpf_cnpj LIKE :cpf LIMIT 50;')
                res3 = conn.execute(q3, {'cpf': f'%{cpf}%'}).mappings().all()
                if res3:
                    for r in res3:
                        print(f" id={r['id']} | {r['nome_razao_social']} | cpf={r['cpf_cnpj']} | tenant={r['tenant_id']} | user={r['user_id']}")
                else:
        ...
        print('Erro de conexão/consulta:', e)
